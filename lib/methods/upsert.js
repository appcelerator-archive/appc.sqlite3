...
exports.upsert = function upsert(Model, id, doc, callback) {
    ...
    var id = parseInt(id);
    persist.findOneById(self.db, table_name, id, doc, function (err, result) {
        if (err) {
            callback(err);
        }
        else {
            if (result === undefined) {  //creates the model, if it is not found
                ...
                persist.insert(table_name, data, self.db, doc, Model, function (err, result) {
                    callback(null, result);
                })
            }
            else {
                persist.update(table_name, id, self.db, doc, callback);
            }
        }
    })
};
