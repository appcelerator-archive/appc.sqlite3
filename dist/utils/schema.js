...
function _changeType(type) {
  var integerTypes = ['INT', 'INTEGER', 'TINYINT', 'SMALLINT', 'MEDIUMINT', 'BIGINT', 'UNSIGNED BIG INT', 'INT2', 'INT8', 'NUMERIC'];
  var stringTypes = ['CHARACTER', 'VARCHAR', 'VARYING CHARACTER', 'NCHAR', 'NATIVE CHARACTER', 'NVARCHAR', 'TEXT', 'CLOB'];
  var doubleTypes = ['REAL', 'DOUBLE', 'DOUBLE PRECISION', 'FLOAT', 'DECIMAL'];
  ...
  if (integerTypes.includes(type.toUpperCase())) {
    ...
    return 'boolean';
  } else if (type.toUpperCase() === 'DATE' || type.toUpperCase() === 'DATETIME') {
    return 'date';
  } else {
    return 'string';
  }
